...
export default function HistoryClient({ userRole }: { userRole: string }) {
  ...
  // Édition de la date d'une entrée (correction) — même périmètre que le commentaire.
  const canEditDates = userRole === "ADMIN" || userRole === "EDITOR";
  const [entries, setEntries] = useState<Entry[] | null>(null);
  const [deleteTarget, setDeleteTarget] = useState<Entry | null>(null);
  const [dateTarget, setDateTarget] = useState<Entry | null>(null);
                        ...
                        {canEditDates && (
                          <button
                            ...
                            className={`flex-1 inline-flex items-center justify-center px-3 text-slate-400 hover:bg-indigo-50 hover:text-indigo-600 transition-colors ${
                              hasEditButton || isAdmin ? "border-b border-slate-100" : ""
                            }`}
                          >
          ...
          onSaved={(nextAt) => {
            setEntries((prev) => {
              ...
              const updated = prev.map((it) =>
                it.type === dateTarget.type && it.id === dateTarget.id
                  ? { ...it, at: nextAt }
                  : it
              );
              // Re-tri décroissant : le regroupement par jour reste chronologique.
              updated.sort((a, b) => (a.at < b.at ? 1 : a.at > b.at ? -1 : 0));
              return updated;
            });
            setDateTarget(null);
...
/**
 * Modale d'édition de la DATE d'une entrée d'historique (PC uniquement — le
 * bouton calendrier est en `hidden lg:flex`).
 *
 * - PATCH `/api/admin/history/[type]/[id]/date` (type mappé via `mapApiType`).
 * - `<input datetime-local>` en heure locale ↔ ISO (UTC) pour l'API.
 * - ADMIN / EDITOR (revérifié côté serveur). Le parent patche `at` localement
 *   et re-trie la liste avant `router.refresh()`.
 */
function EditDateDialog({
  ...
  const initial = toLocal(target.at);
  const [value, setValue] = useState(initial);
  const [busy, setBusy] = useState(false);
  const [error, setError] = useState<string | null>(null);
  ...
  async function submit() {
    if (!valid || !changed || busy || !parsed) return;
    setBusy(true);
    setError(null);
    try {
      const res = await fetch(
        `/api/admin/history/${mapApiType(target.type)}/${target.id}/date`,
        {
          method: "PATCH",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({ date: parsed.toISOString() }),
        }
      );
      if (res.ok) {
        const j = (await res.json().catch(() => ({}))) as { at?: string };
        onSaved(j.at ?? parsed.toISOString());
        return;
      }
      if (res.status === 401 || res.status === 403) {
        setError("Action non autorisée.");
    ...
    <>
      ...
      <div className="fixed top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 z-50 bg-white rounded-xl shadow-2xl border border-slate-200 w-full max-w-md overflow-hidden mx-4">
        <header className="px-4 py-3 border-b border-slate-200">
          ...
          <div className="text-base font-bold">Modifier la date</div>
        </header>
        <div className="p-4 space-y-3">
          <div className="text-xs text-slate-600 border border-slate-200 rounded-lg px-3 py-2">
            <div className="font-mono font-semibold uppercase tracking-wider mb-0.5 text-[10px] text-slate-500">
            ...
            <div className="text-[11px] text-slate-500 mt-0.5">
              Date actuelle :{" "}
              {format(new Date(target.at), "EEEE d MMMM yyyy · HH:mm", {
                locale: fr,
              })}
            </div>
          </div>
